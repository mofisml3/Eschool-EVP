...
import { loadExperimentsInteractionsTrend } from '@/services/dataService';
...
export function InteractionsTrendCard() {
  const loader = useCallback(() => loadExperimentsInteractionsTrend(), []);
  const { state, reload } = useAsyncData<TrendPayload>(loader);

  if (state.status === 'loading') {
    return (
      <ChartCard title={t('experiments.interactionsTrend.title')}>
        <LoadingState />
      </ChartCard>
    );
  }
...
function Inner({ data }: { data: TrendPayload }) {
  return (
    <ChartCard
      title={t('experiments.interactionsTrend.title')}
      insight={t('experiments.interactionsTrend.insight')}
      source={t('experiments.interactionsTrend.source')}
      rightSlot={
        <span className="text-xs text-ink-500 whitespace-nowrap">
          {t('experiments.interactionsTrend.period')}
        </span>
      }
    >
      <div className="h-72 w-full" dir="ltr">
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            <defs>
              <linearGradient id="experiments-trend-fill" x1="0" y1="0" x2="0" y2="1">
                ...
                return (
                  ...
                  >
                    ...
                    <p className="m-0 mt-1 font-semibold text-brand-primary">
                      ...
                      {t('experiments.interactionsTrend.unit')}
                    </p>
                  </div>
                );
              }}
            ...
            <Area
              ...
              stroke="#08798C"
              strokeWidth={2.5}
              fill="url(#experiments-trend-fill)"
